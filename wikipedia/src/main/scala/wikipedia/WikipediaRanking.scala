...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    val seqMethod = (value: Int, article: WikipediaArticle) => {
      if (article.mentionsLanguage(lang)) {
        value + 1
      }
      else value
    }
    val aggMethod = (value1: Int, value2: Int) => {
      value1 + value2
    }
    rdd.aggregate(0)(seqMethod, aggMethod);
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(item => (item._1, item._2.size)).collect().toList.sortBy(_._2)(Ordering.Int.reverse);
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(article => langs.filter(lang => article.mentionsLanguage(lang)).map((_, 1))).reduceByKey(_ + _).collect().toList.sortBy(_._2)(Ordering.Int.reverse);
  }
